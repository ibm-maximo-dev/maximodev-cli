...
template_installers.installTemplateMbo = function(template, dir, templateArgs) {
  dir = dir || env.addonDir() || '.';
  env.ensureDir(dir);
  ...
  shell.ls("-R", tdir).forEach(function(f) {
    if (!fs.lstatSync(path.join(tdir, f)).isDirectory()) {
      template_installers.installTemplateMboFile(path.resolve(tdir, f), dir, f, templateArgs);
    }
  });
};
